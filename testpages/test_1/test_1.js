...
import {uploadUserAnswer,uploadUserAnswer_1} from '../../../utils/test_data_upload_utils.js'
...
Page({
  ...
  onLoad(options) {
    ...
    this.setData({ nextButtonDisabled: true }); // 禁用“下一题”按钮  
    setTimeout(() => {  
      this.setData({ nextButtonDisabled: false }); // 2秒后启用按钮  
    }, 6000); 
  ...
  //点击“下一题”图标时的函数
  onNext() {
    if (!this.data.nextButtonDisabled){
     ...
     //进度百分数+10，并清空输入框中内容
      ...
      if (new_prog == 100) {
        ...
        uploadUserAnswer(this.data.scene_num, this.data.userSentence) }//上传用户答案到Leancloud
          ...
          uploadUserAnswer_1(this.data.scene_num, this.data.userSentence,this.data.id)
        }
      ...
      wx.redirectTo({
        url: '/tests/testpages/test_2/test_2?origin='+this.data.origin+'&id='+this.data.id,
      })
    }}
  },
